...
export function HistoryUsers() {
    ...
    const fetchData = async () => {
        try {
            const response = await MakeRequest(`${process.env.REACT_APP_BASE_URL}/api/v1/material/listMaterialsRepairedByAll`, 'GET');
            if (response) {
                setData(response);
                setRecords(response);
            }
    ...
    const columns = [
        ...
        },
        {
            ...
            cell: row => (
                <>
                    {
                        userData.permissions.includes('edit_materials') && <button className="w-auto mr-1 " onClick={() => handleModify(row)}>
            ...
            <main>
                ...
                <div className="">
                    <div className="flex-row row justify-content-between m-auto align-items-center">
                        <h3>Liste Des Réparations Des Techniciens</h3>
                    </div>
                    <div>
